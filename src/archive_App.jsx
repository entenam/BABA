...
export default function App() {
  ...
  const [editPayForm,  setEditPayForm]  = useState({ type:"Contribution Fee", amount:"", date:"", notes:"" });
  const [confirmPayId, setConfirmPayId] = useState(null);

  const [editingExpId, setEditingExpId] = useState(null);
  const [editExpForm,  setEditExpForm]  = useState({ type:"Court Fee", amount:"", date:"", notes:"" });
  ...
  const startEditPay = pay => {
    ...
    setEditPayForm({ type:pay.type, amount:String(pay.amount), date:pay.date, notes:pay.notes });
    setConfirmPayId(null); setLogFor(null);
  };
  const saveEditPay = () => {
    if (!editPayForm.amount) return;
    persist({ ...data, payments: payments.map(p => p.id === editingPayId ? { ...p, type:editPayForm.type, amount:parseFloat(editPayForm.amount), date:editPayForm.date, notes:editPayForm.notes } : p) });
    setEditingPayId(null);
  };

  const startEditExp = exp => {
    setEditingExpId(exp.id);
    setEditExpForm({ type:exp.type, amount:String(exp.amount), date:exp.date, notes:exp.notes });
    setConfirmExpId(null);
  };
  const saveEditExp = () => {
    if (!editExpForm.amount) return;
    persist({ ...data, expenses: expenses.map(e => e.id === editingExpId ? { ...e, type:editExpForm.type, amount:parseFloat(editExpForm.amount), date:editExpForm.date, notes:editExpForm.notes } : e) });
              ...
              return (
                <div key={player.id} style={{ ...card, marginBottom:12 }}>
                  ...
                  {pPays.map((pay, i) => {
                    ...
                    const border = i < pPays.length-1 ? "0.5px solid var(--color-border-tertiary)" : "none";

                    if (isEditing) return (
                      <div key={pay.id} style={{ padding:"1rem", borderBottom:border, background:"var(--color-background-secondary)" }}>
                        <div style={{ fontSize:12, fontWeight:500, color:"var(--color-text-secondary)", marginBottom:10 }}>Edit payment</div>
                        <Field label="Payment type">
                          <select value={editPayForm.type} onChange={e => setEditPayForm({...editPayForm, type:e.target.value})} style={iStyle}>
                        ...
                        <div style={{ display:"flex", gap:8 }}>
                          <div style={{ flex:1 }}>
                            <Field label="Amount (AUD)">
                              <input type="number" autoFocus value={editPayForm.amount} onChange={e => setEditPayForm({...editPayForm, amount:e.target.value})} style={iStyle} />
                            </Field>
                          </div>
                          <div style={{ flex:1 }}>
                            <Field label="Date">
                              <input type="date" value={editPayForm.date} onChange={e => setEditPayForm({...editPayForm, date:e.target.value})} style={iStyle} />
                            </Field>
                          </div>
                        </div>
                        <Field label="Notes (optional)">
                          <input placeholder="Any extra info" value={editPayForm.notes} onChange={e => setEditPayForm({...editPayForm, notes:e.target.value})} style={iStyle} />
                        </Field>
                        <div style={{ display:"flex", gap:8 }}>
                          <button onClick={() => setEditingPayId(null)} style={{ flex:1 }}>Cancel</button>
                          <button onClick={saveEditPay} style={{ flex:2, fontWeight:500 }}>Save changes ↗</button>
                        </div>
                      </div>
                    );

                    if (isConfirm) return (
                      <div key={pay.id} style={{ padding:"12px 1rem", borderBottom:border, background:"var(--color-background-danger)", display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, flexWrap:"wrap" }}>
                        <span style={{ fontSize:13, color:"var(--color-text-danger)" }}>Remove this payment?</span>
                        <div style={{ display:"flex", gap:8, flexShrink:0 }}>
                          <button onClick={() => setConfirmPayId(null)} style={{ padding:"6px 14px", fontSize:13 }}>Keep</button>
                          <button onClick={() => { removePayment(pay.id); setConfirmPayId(null); }} style={{ padding:"6px 14px", fontSize:13, color:"var(--color-text-danger)", border:"0.5px solid var(--color-border-danger)" }}>Delete</button>
                        </div>
                      </div>
                    );

                    return (
                      <div key={pay.id} style={{ padding:"10px 1rem", borderBottom:border, display:"flex", justifyContent:"space-between", alignItems:"center", gap:8 }}>
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:6, flexShrink:0 }}>
                          ...
                          <button onClick={() => startEditPay(pay)} aria-label="Edit payment"
                            style={{ background:"none", border:"none", cursor:"pointer", color:"var(--color-text-secondary)", padding:"0 4px", lineHeight:1 }}>
                            <i className="ti ti-pencil" style={{ fontSize:13 }} aria-hidden></i>
                          </button>
                          <button onClick={() => { setConfirmPayId(pay.id); setEditingPayId(null); }} aria-label="Delete payment"
                            style={{ background:"none", border:"none", cursor:"pointer", color:"var(--color-text-danger)", padding:"0 4px", lineHeight:1 }}>
                            <i className="ti ti-trash" style={{ fontSize:13 }} aria-hidden></i>
                          </button>
                        </div>
                      </div>
                    );
            ...
            {expenses.length > 0 && (
              <div style={{ ...card, marginBottom:12 }}>
                {expenses.map((exp, i) => {
                  ...
                  const border = i < expenses.length-1 ? "0.5px solid var(--color-border-tertiary)" : "none";

                  if (isEditing) return (
                    <div key={exp.id} style={{ padding:"1rem", borderBottom:border, background:"var(--color-background-secondary)" }}>
                      <div style={{ fontSize:12, fontWeight:500, color:"var(--color-text-secondary)", marginBottom:10 }}>Edit expense</div>
                      <Field label="Expense type">
                        <select value={editExpForm.type} onChange={e => setEditExpForm({...editExpForm, type:e.target.value})} style={iStyle}>
                          {PAYMENT_TYPES.map(t => <option key={t}>{t}</option>)}
                        </select>
                      </Field>
                      <div style={{ display:"flex", gap:8 }}>
                        <div style={{ flex:1 }}>
                          <Field label="Amount (AUD)">
                            <input type="number" autoFocus value={editExpForm.amount} onChange={e => setEditExpForm({...editExpForm, amount:e.target.value})} style={iStyle} />
                          </Field>
                        </div>
                        <div style={{ flex:1 }}>
                          <Field label="Date">
                            <input type="date" value={editExpForm.date} onChange={e => setEditExpForm({...editExpForm, date:e.target.value})} style={iStyle} />
                          </Field>
                        </div>
                      </div>
                      <Field label="Notes (optional)">
                        <input placeholder="e.g. paid on 18th June" value={editExpForm.notes} onChange={e => setEditExpForm({...editExpForm, notes:e.target.value})} style={iStyle} />
                      </Field>
                      <div style={{ display:"flex", gap:8 }}>
                        <button onClick={() => setEditingExpId(null)} style={{ flex:1 }}>Cancel</button>
                        <button onClick={saveEditExp} style={{ flex:2, fontWeight:500 }}>Save changes ↗</button>
                      </div>
                    </div>
                  );

                  if (isConfirm) return (
                    <div key={exp.id} style={{ padding:"12px 1rem", borderBottom:border, background:"var(--color-background-danger)", display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, flexWrap:"wrap" }}>
                      <span style={{ fontSize:13, color:"var(--color-text-danger)" }}>Remove this expense?</span>
                      <div style={{ display:"flex", gap:8, flexShrink:0 }}>
                        <button onClick={() => setConfirmExpId(null)} style={{ padding:"6px 14px", fontSize:13 }}>Keep</button>
                        <button onClick={() => { removeExpense(exp.id); setConfirmExpId(null); }} style={{ padding:"6px 14px", fontSize:13, color:"var(--color-text-danger)", border:"0.5px solid var(--color-border-danger)" }}>Delete</button>
                      </div>
                    </div>
                  );

                  return (
                    <div key={exp.id} style={{ padding:"12px 1rem", borderBottom:border, display:"flex", justifyContent:"space-between", alignItems:"center", gap:8 }}>
                      <div>
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:6, flexShrink:0 }}>
                        ...
                        <button onClick={() => startEditExp(exp)} aria-label="Edit expense"
                          style={{ background:"none", border:"none", cursor:"pointer", color:"var(--color-text-secondary)", padding:"0 4px", lineHeight:1 }}>
                          <i className="ti ti-pencil" style={{ fontSize:14 }} aria-hidden></i>
                        </button>
                        <button onClick={() => { setConfirmExpId(exp.id); setEditingExpId(null); }} aria-label="Delete expense"
                          style={{ background:"none", border:"none", cursor:"pointer", color:"var(--color-text-danger)", padding:"0 4px", lineHeight:1 }}>
